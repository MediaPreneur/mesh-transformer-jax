        ...
        if self.version == 1:
            res.extend(
                node.write_ckpt.remote(
                    f"gs://{bucket}/{path}/step_{step}/", shard_id
                )
                for shard_id, node in zip(
                    range(self.mp), itertools.cycle(self.nodes)
                )
            )

        elif self.version == 2:
            res.extend(
                node.write_ckpt.remote(f"gs://{bucket}/{path}/step_{step}", 0)
                for node in self.nodes
            )
